"""
脚本用途:
- 从 Jira 按 JQL 拉取已解决(Resolved)但仍需验证/同步的缺陷列表(SIT/UAT 环境)
- 生成飞书消息(交互卡或 post 文本),支持按人员聚合、优先级与创建时间排序
- 支持基于映射的真实 @ 提醒(QA 显示名 → 飞书 user_id),并提供详尽日志便于排查

关键功能点:
- SETTINGS 中可配置 Jira 基础信息、消息类型、字段路径与版式、是否开启 @ 提醒等
- 当 MESSAGE_TYPE=interactive:先推送卡片;如开启 AT_QA_MENTION 再补发一条独立 post 进行 @
- 当 MESSAGE_TYPE=post:在主消息正文内联插入 @ 人员块
- @ 提醒仅对 TOP QA(按 QA_FIELD_PATH 聚合计数后取前 AT_TOP_LIMIT)执行,避免噪声
"""
import logging
import sys
import time
...
def _today_title(base_title: str, tz_name: str) -> str:
    ...
    生成带日期前缀的消息标题
    - base_title:基础标题
    - tz_name:时区名称(如 Asia/Shanghai),异常时回退本地时间
    """
    try:
...
SETTINGS: Dict[str, Any] = {
    ...
    "LARK_WEBHOOK_URL": "https://open.larksuite.com/open-apis/bot/v2/hook/164c6cdc-5b26-4b8e-9464-649189d071ff",
    "REPORT_PROJECT_KEYS": ["MP"],
    "REPORT_JQL": "project in (CS, MP) AND issuetype = Bug AND status in ( Resolved) AND Env in (UAT, SIT) AND created >= 2025-01-01 AND created <= 2030-12-31",
    ...
    "MAX_RESULTS": 100,
    "ENABLE_LINKS": True,
    "QA_FIELD_PATH": "reporter.displayName",  # 从 issue.fields 中提取 QA 显示名的路径
    "ENV_FIELD_PATH": "customfield_10701.value",
    "ROW_PREFIX_EMOJI": "-",
    "COUNT_EMOJI": "COUNT",
    "AT_QA_PLAIN": False,          # 纯文本 @ 展示(不触发飞书提醒)
    "AT_QA_MENTION": True,         # 真实 @(通过 user_id 提醒)
    "AT_TOP_LIMIT": 10,            # TOP QA 人数上限(按计数聚合)
    "QA_MAP_PATH": "qa_map.json",
    "QA_MAP_INLINE": {             # 内嵌 QA 显示名 → 飞书 user_id 映射,优先使用
...
class LarkClient:
    ...
    飞书机器人客户端:
    - send_text:纯文本消息
    - send_post_content:post 类型消息(支持 @ 元素)
    - send_interactive_card:交互卡片
    """
...
def _priority_rank(name: Optional[str]) -> int:
    """
    将优先级名称映射为排序权重(数字越小优先级越高):
    - 支持 P0~P3、英文别名(Highest/High/Medium/Low)
    - 未识别返回 999
    """
    if not name:
        return 999
...
def _collect_top_qas_by_counts(issues: List[Dict[str, Any]], qa_field_path: str, top_limit: int) -> List[str]:
    """
    基于计数聚合选择 TOP QA 名单(长度不超过 top_limit)
    """
    counts = _summarize_by_custom_field(issues, qa_field_path)
    top = list(counts.items())[:max(0, top_limit)]
    return [name for name, _ in top if str(name).strip()]

def _collect_qa_names(issues: List[Dict[str, Any]], qa_field_path: str) -> Set[str]:
    """
    从 issues 列表中收集所有非空的 QA 显示名集合
    """
    names: Set[str] = set()
    for issue in issues:
        fields = issue.get("fields") or {}
        val = _get_by_path(fields, qa_field_path)
...
def _get_qa_map(path: str) -> Dict[str, str]:
    """
    获取映射:若 SETTINGS 中存在内嵌映射则优先使用,否则回退到文件
    """
...
def _send_mentions(lark: "LarkClient", issues: List[Dict[str, Any]], qa_field_path: str, qa_map_path: str) -> None:
    """
    发送独立的 post 提醒消息:
    - 仅对 TOP QA(按计数聚合)进行真实 @ 提醒
    - 记录详尽日志;若无 user_id 命中则回退发送名单文本
    """
    ...
    if not user_ids:
        ...
        if names:
            fallback: List[List[Dict[str, Any]]] = [[{"tag": "text", "text": "未匹配到user_id的人员:"}]]
            for n in names:
                fallback[0].append({"tag": "text", "text": n})
            try:
                resp_fb = lark.send_post_content(_today_title("Mention", SETTINGS.get("TIMEZONE", "Asia/Shanghai")), fallback)
                logger.info("提及回退响应=%s", resp_fb)
            except Exception as e:
                logger.error("提及回退失败=%s", e)
        return
    content: List[List[Dict[str, Any]]] = [[{"tag": "text", "text": "Pls Pay Attention To The Dev DONE Time:"}]]
    for uid in user_ids:
...
def format_post_report(issues: List[Dict[str, Any]], project_keys: Optional[List[str]] = None, top_n: int = 10, base_url_for_links: Optional[str] = None, qa_field_path: str = "assignee.displayName", env_field_path: str = "", row_prefix_emoji: str = "-", count_emoji: str = "COUNT", at_qa_plain: bool = False) -> tuple:
    """
    生成 post 文本消息:
    - 展示总数、TOP QA 列表
    - 按优先级与创建时间排序,生成每行条目(可带链接、环境、QA、摘要等)
    - at_qa_plain=True 时追加纯文本 @(不触发提醒)
    返回:标题、内容二维数组(飞书 post 结构)
    """
    total = len(issues)
    title = _today_title("SIT/UAT Pending Verified Bugs(Daily Push)", SETTINGS.get("TIMEZONE", "Asia/Shanghai"))
...
def build_interactive_card(issues: List[Dict[str, Any]], title: str, base_url_for_links: Optional[str], qa_field_path: str, env_field_path: str, count_emoji: str, row_prefix_emoji: str, show_limit: int, jira_base_url: str, jql: str, layout: str, key_emoji: str, env_emoji: str, qa_emoji: str, priority_emoji: str, summary_emoji: str, link_emoji: str, target_tz: str) -> Dict[str, Any]:
    """
    构建交互卡片:
    - 概览总数、TOP QA;主体按排序截断至 show_limit
    - 单列/三列/两列布局兼容;可附加摘要与创建时间
    - 若 base_url_for_links 非空则 Key 使用可点击链接
    """
    total = len(issues)
...
def run_job() -> None:
    ...
    - 校验配置与初始化客户端
    - 拉取 Jira issues
    - 根据 MESSAGE_TYPE 决定推送类型(post/interactive)
    - 若开启 AT_QA_MENTION:
      * post 模式在正文内联 @
      * interactive 模式补发独立 post 进行 @
    - 打印/记录响应与错误
    """
    ...
    try:
        ...
        if str(SETTINGS["MESSAGE_TYPE"]).strip().lower() == "post":
            ...
            if SETTINGS.get("AT_QA_MENTION"):
                ...
                names = _collect_top_qas_by_counts(issues, SETTINGS["QA_FIELD_PATH"], int(SETTINGS.get("AT_TOP_LIMIT", 10)))
                user_ids: List[str] = []
                for n in names:
                    uid = qa_map.get(n)
            ...
            logger.info("Lark响应: %s", resp)
            if SETTINGS.get("AT_QA_MENTION"):
                _send_mentions(lark, issues, SETTINGS["QA_FIELD_PATH"], SETTINGS.get("QA_MAP_PATH", "qa_map.json"))
