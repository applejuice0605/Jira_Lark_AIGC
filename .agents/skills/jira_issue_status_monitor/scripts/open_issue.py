...
SETTINGS: Dict[str, Any] = {
    ...
    "LARK_WEBHOOK_URL": "https://open.larksuite.com/open-apis/bot/v2/hook/56663ea1-4524-4e5f-987e-5536940041d3",
    "REPORT_PROJECT_KEYS": ["CS"],
    "REPORT_JQL": "project = CS AND issuetype = Bug AND status in (Open) AND Env in (UAT, SIT) AND created >= 2025-01-01 AND created <= 2029-12-31",
...
def _send_mentions(lark: "LarkClient", issues: List[Dict[str, Any]], assignee_field_path: str, qa_map_path: str) -> None:
    ...
    if not qa_map:
        return
    names = _collect_assignee_names(issues, assignee_field_path)
    user_ids: List[str] = []
    for n in names:
        uid = qa_map.get(n)
    ...
    lark.send_post_content(_today_title("Mention", SETTINGS.get("TIMEZONE", "Asia/Shanghai")), content)
...
def format_post_report(issues: List[Dict[str, Any]], project_keys: Optional[List[str]] = None, top_n: int = 10, base_url_for_links: Optional[str] = None, assignee_field_path: str = "assignee.displayName", env_field_path: str = "", row_prefix_emoji: str = "-", count_emoji: str = "COUNT", AT_ASSIGNEE_PLAIN: bool = False) -> tuple:
    total = len(issues)
    title = _today_title("SIT/UAT Pending Open Bugs(Daily Push)", SETTINGS.get("TIMEZONE", "Asia/Shanghai"))
    ...
    try:
        ...
        if str(SETTINGS["MESSAGE_TYPE"]).strip().lower() == "post":
            ...
            if SETTINGS.get("AT_ASSIGNEE_MENTION"):
                ...
                names = _collect_assignee_names(issues, SETTINGS["ASSIGNEE_FIELD_PATH"])
                user_ids: List[str] = []
                for n in names:
                    uid = qa_map.get(n)
                    if uid and uid not in user_ids:
